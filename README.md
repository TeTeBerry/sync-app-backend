...
Compose 已为 `app` 注入 `MONGODB_URI`、`REDIS_URL=redis://redis:6379`、`SEED_DEMO_DATA=false`（生产不写 demo 数据）。活动目录与 **电音节阵容**（`festival_sessions` / `artist_performances`）会在服务启动时 upsert；也可手动执行 `npm run db:seed-itinerary`。
...
| 变量 | 说明 |
|------|------|
| `CLOUDBASE_RUN_HOST` | **云托管**公网域名（文档用；如 `https://sync-backend-prd-xxxx.sh.run.tcloudbase.com`），对应前端 `TARO_APP_API_BASE_URL` + `/api` |
