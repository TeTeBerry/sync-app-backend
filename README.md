...
UGC 文本（帖/评/私信/现场资讯/AI 用户消息/资料/举报说明等）落库前走微信 `msg_sec_check`（同 `WECHAT_CONTENT_SECURITY_ENABLED` 开关）。
...
| 变量 | 说明 |
|------|------|
| `CLOUDBASE_ENV_ID` | CloudBase 环境 ID；配置后校验客户端提交的 `cloud://` fileID 须属于该环境 |
| `UPLOAD_DIR` | **本地 dev**：静态文件目录，默认 `./uploads` |
| `UPLOAD_PUBLIC_BASE_URL` | **本地 dev**：如 `http://192.168.x.x:3000`（手环等 LAN 联调） |

生产 UGC：**小程序直传 CloudBase**，后端仅存 `cloud://` fileID，无 COS/STS 密钥。  
本地静态路径：`{UPLOAD_PUBLIC_BASE_URL}/uploads/<filename>`（与 `/api` 前缀无关；仅开发兜底）。
