...
| `WECHAT_SUBSCRIBE_COMMENT_TEMPLATE_ID` | 订阅消息：组队帖被评论（可选；与回复模板二选一或共用） |
| `WECHAT_SUBSCRIBE_COMMENT_REPLY_TEMPLATE_ID` | 订阅消息：评论被回复（默认同评论模板） |
| `WECHAT_SUBSCRIBE_FIELD_ACTOR` / `_PREVIEW` / `_ACTIVITY` | 模板字段名（默认 `thing1` / `thing2` / `thing3`） |
