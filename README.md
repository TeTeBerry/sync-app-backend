...
2. 服务端回复 `connected`
3. 客户端发送 `send`：`messages`、`userId`、`userName`、`activityLegacyId`；图片字段为 **上传后的 URL**（`POST /api/uploads/images`，微信 `img_sec_check`）；不支持 data URL / 外链
