...
import { TicketListingPresenter } from './ticket-listing.presenter';
...
export class TicketListingService {
  private readonly presenter = new TicketListingPresenter();
  ...
  async createFromDraft(
    ...
    return {
      text: this.presenter.buildSuccessReply(draftForReply, displayEventName),
      ticketId,
    };
  }
  ...
  private async tryShowOppositeMatches(
    ...
    return {
      text: this.presenter.buildMatchesReply(
  ...
  async processListingFlow(ctx: ReplyContext): Promise<TicketListingProcessResult> {
    ...
    if (isTicketDraftComplete(draft, accountPhone)) {
      return {
        text: this.presenter.buildRecap(draft, activityName, draftMeta),
        nextState: {
          ...ctx.state,
          ticketListing: {
            ...listing,
    ...
    return {
      text: this.presenter.buildCollectingReply(
        draft,
        activityName,
        draftMeta,
