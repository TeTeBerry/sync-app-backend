...
| 区域 | 状态 | 说明 |
|------|------|------|
| AI 聊天 SSE | ✅ | 前后端事件类型一致；流式 delta / message_complete / done 正常 |
...
| 健康检查 | ✅ | `GET /api/health` → mongodb/redis/chroma 快照 |
...
curl http://localhost:3000/api/health  # {"ok":true,"mongodb":"up","redis":"disabled","chroma":"disabled"}
```
...
8. **健康检查**：`curl http://localhost:3000/api/health`，确认 `mongodb: up` 及 chroma/redis 状态符合预期
